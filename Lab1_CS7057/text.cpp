...
bool load_font_meta (const char* meta_file) {
	...
	printf("Successfully Scanned in Font\n");
	fclose (fp);
	return true;
}
...
bool hide_text(int id){
	...
	renderable_texts[id].visible = false;
	return true;
}

bool unhide_text(int id){

	renderable_texts[id].visible = true;
	return true;
}
